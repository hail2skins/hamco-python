...
def create(request):
    # Protect the view from non-superusers allowing for other apps to be accessed by non-superusers
...
def update(request, pk):  
    ...
    try:
        # get the note by primary key
        note = Note.objects.get(id=pk)
        # Protect the view from non-superusers allowing for other apps to be accessed by non-superusers and also to request.user
